...
static int unicode_to_utf8(unsigned int codepoint, char *p, char **endp) {
  ...
  else if (codepoint < 0x10000) *p++ = 224 + codepoint / 4096, *p++ = 128 + codepoint / 64 % 64,
    *p++ = 128 + codepoint % 64;
  else if (codepoint < 0x110000) *p++ = 240 + codepoint / 262144, *p++ = 128 + codepoint / 4096 % 64,
      ...
      switch (*p) {
        ...
        case 'u': // unicode
          ...
          if ((codepoint & 0xfc00) == 0xd800) { // high surrogate; need one more unicode to succeed
            p += 6;
            if (p[-1] != '\\' || *p != 'u' || (h1 = hex_val(p[1])) < 0 || (h2 = hex_val(p[2])) < 0 || (h3 = hex_val(p[3])) < 0
                || (h4 = hex_val(p[4])) < 0) {
...
nx_json *nx_json_parse_utf8(char *text) {
  return nx_json_parse(text, unicode_to_utf8);
}

nx_json *nx_json_parse(char *text, nx_json_unicode_encoder encoder) {
  nx_json js = {0};

...
#define FDBHEADSIZ     256               // size of the region of the header
...
#define FDBOPAQUESZ    FDBHEADSIZ -  FDBOPAQUEOFF // size of opaque data
#define FDBXFSIZINC    1048576           // 1MB increment of extra file size
...
//pointer to the array region
#define FDBARR(TC_fdb) ((unsigned char *) ((TC_fdb)->map + FDBHEADSIZ))
#define FDBRP(TC_fdb, TC_id) FDBARR(TC_fdb) + ((TC_id) - 1) * ((TC_fdb)->rsiz)
...
enum {
    FDBWRITENOWALL = 1, //do not write into wal file
    FDBWRITENOLOCK = 1 << 1, //do not use shared write lock in tcfdbftruncate
    FDBTRALLOWSHRINK = 1 << 2 //resize file on truncation
};
...
EJDB_INLINE bool tcfdbunlockmethod(TCFDB *fdb);
EJDB_INLINE bool tcfdblockattr(TCFDB *fdb);
EJDB_INLINE bool tcfdbunlockattr(TCFDB *fdb);
EJDB_INLINE bool tcfdblockrecord(TCFDB *fdb, bool wr, uint64_t id);
EJDB_INLINE bool tcfdbunlockrecord(TCFDB *fdb, uint64_t id);
EJDB_INLINE bool tcfdblockallrecords(TCFDB *fdb, bool wr);
EJDB_INLINE bool tcfdbunlockallrecords(TCFDB *fdb);
EJDB_INLINE bool tcfdblockwal(TCFDB *fdb);
EJDB_INLINE bool tcfdbunlockwal(TCFDB *fdb);
EJDB_INLINE bool tcfdblocksmem(TCFDB *fdb, bool wr);
EJDB_INLINE bool tcfdblocksmem2(TCFDB *fdb, bool wr);
EJDB_INLINE bool tcfdbunlocksmem(TCFDB *fdb);
...
bool tcfdbsetmutex(TCFDB *fdb) {
    ...
    TCMALLOC(fdb->smtx, sizeof (pthread_rwlock_t));
    TCMALLOC(fdb->eckey, sizeof (pthread_key_t));
    bool err = false;
    if (pthread_rwlock_init(fdb->mmtx, NULL) != 0) err = true;
    if (pthread_rwlock_init(fdb->smtx, NULL) != 0) err = true;
...
bool tcfdbcopyopaque(TCFDB *dst, TCFDB *src, int off, int rsz) {
    assert(dst && src);
    if (rsz == -1) {
        rsz = FDBOPAQUESZ;
    }
    char odata[FDBOPAQUESZ];
    rsz = tcfdbreadopaque(src, odata, off, rsz);
    if (rsz == -1) {
        tcfdbsetecode(dst, tcfdbecode(src), __FILE__, __LINE__, __func__);
        return false;
    }
    return (tcfdbwriteopaque(dst, odata, off, rsz) == rsz);
}
...
int tcfdbreadopaque(TCFDB *fdb, void *dst, int off, int bsiz) {
    ...
    bsiz = MIN(bsiz, FDBOPAQUESZ - off);
    if (bsiz <= 0) {
        tcfdbsetecode(fdb, TCEINVALID, __FILE__, __LINE__, __func__);
        return -1;
    }
    if (!FDBLOCKSMEM(fdb, false)) return -1;
    assert(fdb->map);
    memcpy((void *) dst, (void *) (fdb->map + FDBOPAQUEOFF + off), bsiz);
    FDBUNLOCKSMEM(fdb);
    return bsiz;
}
...
int tcfdbwriteopaque(TCFDB *fdb, const void *src, int off, int nb) {
    ...
    nb = MIN(nb, FDBOPAQUESZ - off);
    if (nb <= 0) {
        tcfdbsetecode(fdb, TCEINVALID, __FILE__, __LINE__, __func__);
        return -1;
    }
    if (!FDBLOCKSMEM(fdb, true)) return -1;
    memcpy((void *) (fdb->map + FDBOPAQUEOFF + off), src, nb);
    FDBUNLOCKSMEM(fdb);
    return nb;
}
...
static void tcfdbsetflag(TCFDB *fdb, int flag, bool sign) {
    assert(fdb);
    if (!FDBLOCKSMEM(fdb, true)) return;
...
static bool tcfdbwalinit(TCFDB *fdb) {
    ...
    FDBUNLOCKWAL(fdb);
    fdb->walend = fdb->fsiz;
    if (!tcfdbwalwrite(fdb, 0, FDBHEADSIZ)) {
        return false;
    }
    return true;
}
...
static bool tcfdbwalwrite(TCFDB *fdb, uint64_t off, int64_t size) {
    ...
    wp += sizeof (lnum);
    memcpy(wp, (void *) (fdb->map + off), size);
    wp += size;
    if (!FDBLOCKWAL(fdb)) return false;
...
static int tcfdbwalrestore(TCFDB *fdb, const char *path) {
    ...
    if (!(fdb->omode & FDBOWRITER) || walsiz < sizeof (walsiz) + FDBHEADSIZ) {
        FDBUNLOCKWAL(fdb);
        err = true;
        goto finish;
    }
    ...
    if (FDBLOCKSMEM(fdb, true)) {
        for (int i = TCLISTNUM(list) - 1; i >= 0; i--) {
            ...
            rec += sizeof (off);
            size -= sizeof (off);
            if (!tcfseek(fdb->fd, off, TCFSTART)) {
                tcfdbsetecode(fdb, TCESEEK, __FILE__, __LINE__, __func__);
                err = true;
                break;
            }
            if (!tcwrite(fdb->fd, rec, size)) {
                tcfdbsetecode(fdb, TCEWRITE, __FILE__, __LINE__, __func__);
                err = true;
                break;
            }
        }
        if (!tcfdbftruncate2(fdb, fsiz, FDBWRITENOLOCK)) {
            tcfdbsetecode(fdb, TCETRUNC, __FILE__, __LINE__, __func__);
            err = true;
        }
        char hbuf[FDBHEADSIZ];
    ...
    if (!tcfdbmemsync(fdb, (fdb->omode & FDBOTSYNC))) {
        tcfdbsetecode(fdb, TCESYNC, __FILE__, __LINE__, __func__);
        err = true;
    }
finish:
...
static bool tcfdbopenimpl(TCFDB *fdb, const char *path, int omode) {
    ...
    if ((omode & FDBOWRITER) && (omode & FDBOTRUNC)) {
        if (!tcfdbftruncate2(fdb, 0, FDBTRALLOWSHRINK)) {
            tcfdbsetecode(fdb, TCETRUNC, __FILE__, __LINE__, __func__);
            CLOSEFH2(fdb->fd);
            return false;
        }
    ...
    char hbuf[FDBHEADSIZ];
    if ((omode & FDBOWRITER) && (sbuf.st_size < 1 || (omode & FDBOTRUNC))) {
        fdb->flags = 0;
        fdb->rnum = 0;
        fdb->fsiz = FDBHEADSIZ;
        fdb->min = 0;
...
#ifndef _WIN32
    void *map = mmap(0, fdb->limsiz,
            (PROT_READ | ((omode & FDBOWRITER) ? PROT_WRITE : 0)),
            MAP_SHARED, fd, 0);
    if (map == MAP_FAILED) {
        tcfdbsetecode(fdb, TCEMMAP, __FILE__, __LINE__, __func__);
        CLOSEFH2(fdb->fd);
    ...
    fdb->map = map;
#else
    if (!tcfdbftruncate(fdb, sbuf.st_size)) {
        tcfdbsetecode(fdb, TCETRUNC, __FILE__, __LINE__, __func__);
        CLOSEFH2(fdb->fd);
        fdb->fd = INVALID_HANDLE_VALUE;
        return false;
...
static bool tcfdbcloseimpl(TCFDB *fdb) {
    ...
    if (fdb->omode & FDBOWRITER) {
        if (!err && !tcfdbftruncate2(fdb,
                ((uint64_t) FDBHEADSIZ + (fdb->max > 0 ? fdb->max + 1 : 0) * fdb->rsiz),
                FDBTRALLOWSHRINK)) {
            tcfdbsetecode(fdb, TCETRUNC, __FILE__, __LINE__, __func__);
            err = true;
        }
    ...
    if (!FDBLOCKSMEM2(fdb, true)) err = true;
#ifndef _WIN32
    ...
    if (!INVALIDHANDLE(fdb->walfd)) {
        ...
        if (!fdb->fatal && !tcfdbwalremove(fdb, fdb->path)) {
            err = true;
        }
    }
    if (!INVALIDHANDLE(fdb->fd) && !CLOSEFH(fdb->fd)) {
        tcfdbsetecode(fdb, TCECLOSE, __FILE__, __LINE__, __func__);
        err = true;
    }
    TCFREE(fdb->path);
...
static int64_t tcfdbprevid(TCFDB *fdb, int64_t id) {
    ...
    while (id >= fdb->min) {
        ...
        unsigned char *rec = FDBRP(fdb, id);
...
static bool tcfdbputimpl(TCFDB *fdb, int64_t id, const void *vbuf, int vsiz, int dmode) {
    ...
    if (nsiz > fdb->fsiz) {
        ...
        //take a WRITE LOCK to change fdb->map in tcfdbftruncate2
        if (!FDBLOCKSMEM(fdb, true)) {
            return false;
        }
        if (!FDBLOCKATTR(fdb)) {
        ...
        if (nsiz > fdb->fsiz) {
            ...
            if (!err && !tcfdbftruncate2(fdb, nsiz, FDBWRITENOLOCK)) {
                tcfdbsetecode(fdb, TCETRUNC, __FILE__, __LINE__, __func__);
                err = true;
            }
            if (!err) {
                TCDODEBUG(fdb->cnt_truncfile++);
                unsigned char *wp = FDBRP(fdb, id);
                uint16_t snum;
                uint32_t lnum;
                switch (fdb->wsiz) {
                    case 1:
    ...
    //we would not change fdb->map address by tcfdbftruncate2
    //and our write area is locked by FDBLOCKRECORD
    if (!FDBLOCKSMEM(fdb, false)) {
        return false;
    }
    unsigned char *rp = FDBRP(fdb, id);
    uint32_t osiz;
    uint16_t snum;
    uint32_t lnum;
    switch (fdb->wsiz) {
    ...
    if (dmode != FDBPDOVER && !miss) {
        ...
        if (dmode == FDBPDCAT) {
            unsigned char *rec = FDBRP(fdb, id);
            if (fdb->tran && !tcfdbwalwrite(fdb, (char *) rec - fdb->map, fdb->width)) {
                err = true;
                goto finish;
            }
...
static bool tcfdboutimpl(TCFDB *fdb, int64_t id) {
    ...
    //we would not change fdb->map address by tcfdbftruncate2
    //and our write area is locked by FDBLOCKRECORD
    if (!FDBLOCKSMEM(fdb, false)) return false;
    bool err = false;
    unsigned char *rec = FDBRP(fdb, id);
    unsigned char *rp = rec;
    uint32_t osiz;
...
EJDB_INLINE bool tcfdblocksmem(TCFDB *fdb, bool wr) {
    assert(fdb);
    if (wr ? pthread_rwlock_wrlock(fdb->smtx) != 0 : pthread_rwlock_rdlock(fdb->smtx) != 0) {
        tcfdbsetecode(fdb, TCETHREAD, __FILE__, __LINE__, __func__);
        return false;
    }
    if (fdb->map == NULL) {
...
EJDB_INLINE bool tcfdbunlocksmem(TCFDB *fdb) {
    assert(fdb);
    if (pthread_rwlock_unlock(fdb->smtx) != 0) {
        tcfdbsetecode(fdb, TCETHREAD, __FILE__, __LINE__, __func__);
        return false;
    }
    TCTESTYIELD();
...
static bool tcfdbftruncate(TCFDB *fdb, off_t length) {
    return tcfdbftruncate2(fdb, length, 0);
}
...
static bool tcfdbftruncate2(TCFDB *fdb, off_t length, int opts) {
    ...
    size.QuadPart = (fdb->omode & FDBOWRITER) ? tcpagealign((length == 0) ? 1 : length) : length;
    if (fdb->map &&
            length > 0 &&
            (!(fdb->omode & FDBOWRITER) || (size.QuadPart <= fdb->fsiz && !(opts & FDBTRALLOWSHRINK)))) {
        return true;
    }
    if (!(opts & FDBWRITENOLOCK) && !FDBLOCKSMEM2(fdb, true)) {
        return false;
    }
    if ((fdb->omode & FDBOWRITER) && size.QuadPart > fdb->fsiz && !(opts & FDBTRALLOWSHRINK)) {
        off_t o1 = tcpagealign((_maxof(off_t) - (off_t) size.QuadPart < FDBXFSIZINC) ? size.QuadPart : size.QuadPart + FDBXFSIZINC);
        off_t o2 = tcpagealign((((uint64_t) size.QuadPart) * 3) >> 1);
        size.QuadPart = MAX(o1, o2);
    }
    if (size.QuadPart > fdb->limsiz) {
    ...
    if (fdb->omode & FDBOWRITER) {
        if (!SetFilePointerEx(fdb->fd, size, NULL, FILE_BEGIN) || !SetEndOfFile(fdb->fd)) {
            tcfdbsetecode(fdb, TCESEEK, __FILE__, __LINE__, __func__);
            err = true;
            goto finish;
        }
    }
    fdb->w32hmap = CreateFileMapping(fdb->fd, NULL,
            ((fdb->omode & FDBOWRITER) ? PAGE_READWRITE : PAGE_READONLY),
            size.HighPart, size.LowPart, NULL);
    if (INVALIDHANDLE(fdb->w32hmap)) {
        tcfdbsetecode(fdb, TCEMMAP, __FILE__, __LINE__, __func__);
        err = true;
        goto finish;
    }
    fdb->map = MapViewOfFile(fdb->w32hmap,
            ((fdb->omode & FDBOWRITER) ? FILE_MAP_WRITE : FILE_MAP_READ),
            0, 0, 0);
    if (fdb->map == NULL) {
        tcfdbsetecode(fdb, TCEMMAP, __FILE__, __LINE__, __func__);
        err = true;
    ...
    fdb->fsiz = size.QuadPart;
    if (length == 0) {
        size.QuadPart = 0;
        if (!SetFilePointerEx(fdb->fd, size, NULL, FILE_BEGIN)) {
            tcfdbsetecode(fdb, TCESEEK, __FILE__, __LINE__, __func__);
            err = true;
            goto finish;
        }
...
void tcfdbprintmeta(TCFDB *fdb) {
    ...
    wp += sprintf(wp, " smtx=%p", (void *) fdb->smtx);

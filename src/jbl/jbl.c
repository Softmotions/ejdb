...
static iwrc _jbl_node_from_binn(JBLDRCTX *ctx, const binn *bn, JBL_NODE parent, char *key, int klidx) {
  binn bv;
  binn_iter iter;
  iwrc rc = 0;
...
static iwrc _jbl_from_node(binn *res, const JBL_NODE node, int size) {
  iwrc rc = 0;
  switch (node->type) {
    case JBV_OBJECT:
...
iwrc jbl_patch_from_json(JBL jbl, const char *patchjson) {
  ...
  rc = jbl_node_from_json(patchjson, &node, pool);
  RCGO(rc, finish);
